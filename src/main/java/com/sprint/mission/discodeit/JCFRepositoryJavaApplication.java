...
public class JCFRepositoryJavaApplication {
    static User setupUser(UserService userService){
        User user = userService.createUser(new User("user01"));
        return user;
    }

    static Channel setupChannel(ChannelService channelService){
        Channel channel = channelService.createChannel(new Channel("JCF*Repository"));
        return channel;
    }

    static Message messageCreateTest(MessageService messageService, Channel channel, User sender){
        Message message = messageService.createMessage(new Message("Hello! JCF*Repository Test!", sender.getId(), channel.getId() ));
